...
const socialLinks = [
  ...
  {
    label: "WhatsApp",
    href: `https://wa.me/${company.phoneTel.replace(/\D/g, "")}`,
    icon: WhatsAppIcon,
    external: true,
  },
];
...
function FooterHeading({ children }: { children: ReactNode }) {
  return (
    <div className="mb-5">
      ...
      <div className="mt-2 h-0.5 w-10 rounded-full bg-amber-400" aria-hidden />
    </div>
  );
}
...
export default function Footer() {
  return (
    <footer className="mt-auto bg-nav-brand text-white">
      <div className="mx-auto grid max-w-6xl gap-10 py-12 pl-2 pr-4 sm:pl-3 sm:pr-6 lg:grid-cols-3 lg:gap-12 lg:pl-4 lg:pr-8">
        {/* Brand + social */}
        <div className="-ml-1 sm:-ml-2">
          <div className="flex items-center gap-2 sm:gap-3">
            <Image
              ...
              width={160}
              height={160}
              className="-ml-2 h-28 w-auto shrink-0 object-contain sm:-ml-3 sm:h-36 lg:-ml-4 lg:h-40"
            />
            <p className="font-display text-base font-bold leading-snug text-white sm:text-lg">
              {company.legalName}
            </p>
          </div>
          <div className="mt-6 flex gap-2 pl-0 sm:pl-1">
            {socialLinks.map(({ label, href, icon: Icon, external }) => {
              const className =
                "flex h-9 w-9 items-center justify-center rounded-md bg-white/15 text-white transition-colors hover:bg-white/25";
        ...
        <div>
          ...
          <ul className="space-y-4 text-sm sm:text-base">
            <li className="flex gap-3">
              ...
              <a href={`tel:${company.phoneTel}`} className="text-white/90 hover:text-white">
                {company.phoneTel}
              </a>
            </li>
            <li className="flex gap-3">
              <MailIcon className="mt-0.5 h-5 w-5 shrink-0 text-white" />
              <a href={`mailto:${company.email}`} className="text-white/90 hover:text-white">
                {company.email}
              </a>
            </li>
            <li className="flex gap-3">
      ...
      <div className="border-t border-white/20 bg-[#3d7a1a] px-4 py-4 text-center text-sm text-white/80 sm:text-base">
